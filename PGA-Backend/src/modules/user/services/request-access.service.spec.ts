...
describe('RequestAccessService', () => {
  ...
  it('deve lançar NotFoundException se unidade não encontrada', async () => {
    ...
    await expect(service.execute({ codigo_unidade: 'X', email: 'a@b.com', nome: 'A' } as any)).rejects.toThrow(NotFoundException);
  });
  ...
  it('deve lançar ConflictException se já existe solicitação pendente', async () => {
    ...
    mockPrisma.solicitacaoAcesso.findFirst.mockResolvedValue({ solicitacao_id: 1 });
    await expect(service.execute({ codigo_unidade: 'X', email: 'a@b.com', nome: 'A' } as any)).rejects.toThrow(ConflictException);
  });

  it('deve criar solicitação com sucesso', async () => {
    mockPrisma.unidade.findFirst.mockResolvedValue({ unidade_id: 1 });
    mockPrisma.solicitacaoAcesso.findFirst.mockResolvedValue(null);
    mockPrisma.solicitacaoAcesso.create.mockResolvedValue({ solicitacao_id: 1, nome: 'A' });
    const result = await service.execute({ codigo_unidade: 'X', email: 'a@b.com', nome: 'A' } as any);
