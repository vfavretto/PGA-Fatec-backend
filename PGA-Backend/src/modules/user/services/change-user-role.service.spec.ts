...
describe('ChangeUserRoleService', () => {
  ...
  it('deve lançar NotFoundException se usuário alvo não encontrado', async () => {
    ...
    await expect(service.execute('uuid-99', 'Diretor' as any)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('deve lançar ForbiddenException se usuário logado sem permissão', async () => {
    ...
    await expect(
      service.execute('uuid-1', 'Diretor' as any, undefined, 'uuid-2'),
  ...
  it('deve continuar sem atualizar unidade quando Diretor não dirige nenhuma unidade', async () => {
    mockRepo.findById.mockResolvedValue({
      pessoa_id: 'uuid-1',
      tipo_usuario: 'Diretor',
    });
    mockTx.unidade.findFirst.mockResolvedValue(null); // não é diretor de nenhuma unidade
    mockTx.pessoa.update.mockResolvedValue({ pessoa_id: 'uuid-1', tipo_usuario: 'Docente' });
    await service.execute('uuid-1', 'Docente' as any);
    expect(mockTx.unidade.update).not.toHaveBeenCalled();
  });
  ...
  it('deve lançar ForbiddenException se definindo Diretor sem unidadeId', async () => {
    ...
    await expect(service.execute('uuid-1', 'Diretor' as any)).rejects.toThrow(
      ForbiddenException,
    );
  });
  ...
  it('deve lançar NotFoundException se unidade não encontrada ao definir Diretor', async () => {
    ...
    await expect(
      service.execute('uuid-1', 'Diretor' as any, 'uuid-99'),
  ...
  it('deve lançar ConflictException se unidade já tem diretor diferente', async () => {
    ...
    await expect(
      service.execute('uuid-1', 'Diretor' as any, 'uuid-10'),
