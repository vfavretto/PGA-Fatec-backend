...
describe('ProcessAccessRequestService', () => {
  ...
  it('deve lançar BadRequestException se solicitação não encontrada', async () => {
    ...
    await expect(
      service.execute('uuid-1', 'uuid-1', 'Aprovada'),
  ...
  it('deve lançar ConflictException se unidade já tem diretor', async () => {
    ...
    await expect(
      service.execute('uuid-1', 'uuid-1', 'Aprovada', 'Diretor' as any),
    ).rejects.toThrow(ConflictException);
  });

  it('deve lançar UnauthorizedException quando processador não-Admin tenta criar nível igual/superior', async () => {
    mockPrisma.solicitacaoAcesso.findUnique.mockResolvedValue(mockSolicitacao);
    mockPrisma.pessoa.findUnique.mockResolvedValue({
      pessoa_id: 'uuid-2',
    ...
    await expect(
      service.execute('uuid-1', 'uuid-2', 'Aprovada', 'Regional' as any),
    ).rejects.toThrow(UnauthorizedException);
  });

  it('deve aprovar quando processador Regional solicita nível abaixo do seu', async () => {
    mockPrisma.solicitacaoAcesso.findUnique.mockResolvedValue(mockSolicitacao);
    mockPrisma.pessoa.findUnique.mockResolvedValue({
      pessoa_id: 'uuid-2',
      tipo_usuario: 'Regional',
    ...
    mockPrisma.solicitacaoAcesso.update.mockResolvedValue({});
    mockPrisma.pessoa.create.mockResolvedValue({ pessoa_id: 'uuid-20' });
    mockPrisma.pessoaUnidade.create.mockResolvedValue({});
    mockSendApproved.execute.mockResolvedValue(undefined);

    const result = await service.execute('uuid-1', 'uuid-2', 'Aprovada', 'Docente' as any);
    expect(result.success).toBe(true);
  });
});
