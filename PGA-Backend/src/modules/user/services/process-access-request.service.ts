...
export class ProcessAccessRequestService {
  ...
  async execute(solicitacaoId: number, usuarioId: number, status: string, tipoUsuario?: TipoUsuario) {
    // Buscar solicitação
    ...
    const permissaoProcessar = ['Administrador', 'CPS', 'Regional', 'Diretor'].includes(processador.tipo_usuario);
    
    if (!permissaoProcessar) {
      throw new UnauthorizedException('Você não tem permissão para processar solicitações de acesso');
    }
    ...
    if (processador.tipo_usuario === 'Diretor') {
      ...
      if (!unidadeDiretor || unidadeDiretor.unidade_id !== solicitacao.unidade_id) {
        throw new UnauthorizedException('Você não pode processar solicitações de outras unidades');
      }
    }
    ...
    if (status === 'Aprovada') {
      if (!tipoUsuario) {
        throw new BadRequestException('Tipo de usuário é obrigatório para aprovação');
      }
      ...
      if (nivelSolicitado >= nivelProcessador) {
        throw new UnauthorizedException('Você não pode criar usuários do mesmo nível ou superior ao seu');
      }
    }
    ...
    if (status === 'Aprovada' && tipoUsuario) {
      ...
      // Correção 2: Remover unidade_id do create de pessoa
      const novoUsuario = await this.prisma.pessoa.create({
        data: {
          ...
          ativo: true,
          // Relações precisam ser definidas conforme schema
          // Se for necessário associar à unidade, use a sintaxe correta do Prisma
        }
      });
      
      // Correção 3: Usar nome_completo em vez de nome
