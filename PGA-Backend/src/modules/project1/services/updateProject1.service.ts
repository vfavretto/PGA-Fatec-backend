...
export class UpdateProject1Service {
  ...
  async execute(id: string, data: UpdateProject1Dto) {
    try {
      const { situacoes_problema_ids, pessoas, etapas, ...projectData } = data;
      ...
      console.log(
        '[UpdateProject1Service] Atualizando projeto com dados básicos:',
        { id, basicFields },
      );

      if (Object.keys(basicFields).length > 0) {
        await this.project1Repository.update(
          id,
          basicFields as UpdateProject1Dto,
        );
      }

      if (
        situacoes_problema_ids !== undefined &&
        Array.isArray(situacoes_problema_ids)
      ) {
        console.log('[UpdateProject1Service] Atualizando situações-problema:', {
          id,
        ...
        if (idsValidos.length > 0) {
          console.log(
            '[UpdateProject1Service] Criando situações-problema válidas:',
            { idsValidos },
          );
          ...
          console.log(
            '[UpdateProject1Service] Situações-problema criadas com sucesso',
          );
        }
      }

      if (pessoas !== undefined && Array.isArray(pessoas)) {
        console.log('[UpdateProject1Service] Atualizando pessoas:', {
          id,
          pessoas_count: pessoas.length,
        });

        const pessoasValidas = pessoas.filter((p) => p && p.pessoa_id);
        console.log('[UpdateProject1Service] Pessoas válidas:', {
          count: pessoasValidas.length,
        });
        ...
        for (const pessoa of pessoasValidas) {
          ...
          const cargaHoraria = pessoa.carga_horaria_semanal
            ? Number(pessoa.carga_horaria_semanal)
            : null;

          if (pessoa.projeto_pessoa_id) {
            console.log(
              '[UpdateProject1Service] Atualizando pessoa existente:',
              { projeto_pessoa_id: pessoa.projeto_pessoa_id },
            );
      ...
      if (etapas !== undefined && Array.isArray(etapas)) {
        console.log('[UpdateProject1Service] Atualizando etapas:', {
          id,
        ...
        for (const etapa of etapasValidas) {
          ...
          if (etapa.etapa_id) {
            console.log(
              '[UpdateProject1Service] Atualizando etapa existente:',
            ...
            await this.prisma.etapaProcesso.update({
              ...
              data: {
                ...
                status_verificacao:
                  (etapa.status_verificacao as any) || undefined,
      ...
      console.error(
        '[UpdateProject1Service] Erro ao atualizar projeto:',
        error,
      );
      throw error;
    }
