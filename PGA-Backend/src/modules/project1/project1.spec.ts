...
describe('UpdateProject1Service', () => {
  ...
  it('deve retornar null para data inválida (cobre branch NaN de parseDate)', async () => {
    mockRepo.findOne.mockResolvedValue({ acao_projeto_id: 'uuid-1' });
    mockPrisma.etapaProcesso.create.mockResolvedValue({});
    const etapas = [{ descricao: 'Etapa', data_verificacao_prevista: 'not-a-date' }];
    await service.execute('uuid-1', { etapas } as any);
    expect(mockPrisma.etapaProcesso.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ data_verificacao_prevista: null }),
      }),
    );
  });

  it('deve lançar e relançar erro do catch quando update falha', async () => {
    mockRepo.findOne.mockResolvedValue({ acao_projeto_id: 'uuid-1' });
    mockRepo.update.mockRejectedValue(new Error('DB error'));
    await expect(service.execute('uuid-1', { titulo: 'X' } as any)).rejects.toThrow('DB error');
  });
});
