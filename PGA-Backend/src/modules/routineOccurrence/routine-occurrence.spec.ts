...
describe('CreateRoutineOccurrenceService', () => {
  ...
  beforeEach(() => { jest.clearAllMocks(); service = new CreateRoutineOccurrenceService(mockRepo as any); });

  it('deve criar e retornar a ocorrência de rotina', async () => {
    mockRepo.create.mockResolvedValue({ occurrence_id: 1 });
    expect(await service.execute({ data: '2024-01-01' } as any)).toEqual({ occurrence_id: 1 });
  });
});

describe('FindAllRoutineOccurrenceService', () => {
  let service: FindAllRoutineOccurrenceService;
  beforeEach(() => { jest.clearAllMocks(); service = new FindAllRoutineOccurrenceService(mockRepo as any); });
...
describe('FindOneRoutineOccurrenceService', () => {
  ...
  beforeEach(() => { jest.clearAllMocks(); service = new FindOneRoutineOccurrenceService(mockRepo as any); });
...
describe('UpdateRoutineOccurrenceService', () => {
  ...
  beforeEach(() => { jest.clearAllMocks(); service = new UpdateRoutineOccurrenceService(mockRepo as any); });
...
describe('DeleteRoutineOccurrenceService', () => {
  ...
  beforeEach(() => { jest.clearAllMocks(); service = new DeleteRoutineOccurrenceService(mockRepo as any); });
