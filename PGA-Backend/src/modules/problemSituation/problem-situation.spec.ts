...
describe('CreateProblemSituationService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new CreateProblemSituationService(mockRepo as any);
  });
  ...
  it('deve criar quando código não está em uso', async () => {
    ...
    ).toEqual({ problem_situation_id: 1 });
  });

  it('deve lançar ConflictException se código já está em uso por situação ativa', async () => {
    mockRepo.findByCodigoCategoria.mockResolvedValue({
      situacao_id: 2,
      ativo: true,
    });
    await expect(
      service.execute({ codigo_categoria: 'A1' } as any),
...
describe('FindAllProblemSituationService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new FindAllProblemSituationService(mockRepo as any);
  });
...
describe('FindOneProblemSituationService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new FindOneProblemSituationService(mockRepo as any);
  });

  it('deve retornar situação problema encontrada', async () => {
    mockRepo.findOne.mockResolvedValue({ problem_situation_id: 1 });
    expect(await service.execute('uuid-1')).toEqual({
      problem_situation_id: 1,
    });
  });
...
describe('UpdateProblemSituationService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new UpdateProblemSituationService(mockRepo as any);
  });
  ...
  it('deve atualizar e retornar situação problema', async () => {
    ...
    expect(
      (await service.execute('uuid-1', { descricao: 'Novo' } as any)).descricao,
    ).toBe('Novo');
  });
  ...
  it('deve lançar ConflictException se código em uso por outra situação ativa', async () => {
    ...
    await expect(
      service.execute('uuid-1', { codigo_categoria: 'X1' } as any),
    ).rejects.toThrow(ConflictException);
  });
...
describe('DeleteProblemSituationService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new DeleteProblemSituationService(mockRepo as any);
  });
