...
export class DeleteWorkloadHaeService {
  ...
  async execute(id: number, usuarioLogadoId?: number, motivo?: string) {
    ...
    if (!workloadHae) throw new NotFoundException('Tipo de vínculo HAE não encontrado');
    ...
    if (projetosPessoas > 0) {
      throw new ConflictException('Este tipo de vínculo HAE está sendo usado em vínculos de projeto ativos e não pode ser excluído');
    }
